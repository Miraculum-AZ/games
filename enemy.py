...
import sys
import random
from bullet_enemy import Bullet

#--Creating a tank:
class Enemy(pygame.sprite.Sprite):

	def __init__(self, field_size, pos_x, pos_y):
		super().__init__()
...
#--Creating a random tank out of the list:			
		self.random_tank_enemy = random.choices(self.list_of_tank_enemy_names, weights=[5,4,3,2])
#--Giving it appropriate image and rect:		
		self.image = pygame.image.load('C:/Users/KOMAROVO/Desktop/Code here/Images/Enemies/' + self.random_tank_enemy[0] + '.png').convert_alpha()
...
#--ALL THESE CHARS for ENEMY are based on the random_tank_enemy[0]:
		match self.random_tank_enemy[0]:
			case 'tank_basic': self.level = 1
			case 'tank_fast': self.level = 2
			case 'tank_power': self.level = 3
			case 'tank_armor': self.level = 4
		print(self.level)
		
		self.speed_mode = [0, 2, 2, 3]
		...
		match self.level:
			...
			case 4:
				...
				self.level_speed = 2
				self.gun_barrel_cooldown_time = 1100
